...
export const Button: FC<PropsWithChildren<ButtonProps>> = ({
  ...
  return (
    <button
      type="button"
      className={twMerge(
        "relative py-2 px-3 text-sm font-medium rounded-lg bg-[#0E0024] shadow-[0px_0px_12px_rgba(15,162,218,0.5)]",
        classname,
      )}
    >
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 shadow-[0_0_10px_rgba(15,162,218,0.7)_inset] rounded-lg"></div>
      </div>
      <span>{children}</span>
    </button>
  );
